...
;; Package-Requires: ((emacs "28.1") (compat "30") (el-job) (llama))
...
SYNCHRONOUS t, unless SYNCHRONOUS is the symbol `must-async'."
  ...
  (when (eq t synchronous)
    ;; (length (el-job-results (gethash 'org-node el-jobs)))
    ;; (let ((procs (mapcar #'process-status (el-job-processes (gethash 'org-node el-jobs)))))
    (let ((procs (el-job:busy (gethash 'org-node el-jobs))))
      ;; Block until all processes finish
      (when (seq-some #'process-live-p procs)
        (if org-node-cache-mode
      ...
      ;; (mapc #'accept-process-output procs)
...
(defun org-node--print-elapsed (job)
