...
#+begin_src elisp
(setq org-roam-db-update-on-save nil) ;; don't scan file on save
...
*Option 1A*.  Feed data to org-roam so all its features will Just Work^{sort of}.  For a first-time run, type =M-x org-node-feed-roam-db= (slow!).  Then add the following init snippet to keep the database updated over time.

#+begin_src elisp
(add-hook 'org-node-cache-scan-file-hook #'org-node-feed-file-to-roam-db)
#+end_src

*Option 1B*.  Cut out the DB altogether.  Instead of the above, add this hack:
...
For a first-time run, type =M-x org-node-backlink-fix-all=.  (Don't worry, if you change your mind there is =M-x org-node-backlink-regret=.)
