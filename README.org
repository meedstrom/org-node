...
Also, either run =M-x org-roam-id-update-id-locations=, or edit the following variable to include =org-roam-directory=.  Ideally org-id would manage itself, but it doesn't do it super-well, and this is insurance.
...
The option =org-node-filter-fn= works well for ignoring TODO items that happen to have an ID, and ignoring org-drill items and that sort of thing, but beyond that, it has limited utility because unlike org-roam, *child ID nodes of an excluded node are not excluded!*

So let's say you have a big archive file, fulla IDs, and you want to exclude all of them from the minibuffer completions.  Putting a =:ROAM_EXCLUDE: t= at the top won't do it.  As it stands, what I'd suggest is unfortunately, look at the file name.
