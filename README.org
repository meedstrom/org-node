...
- *Breaking API change* -- nodes are objects now, not plists!  Unfortunately the software can't give you a deprecation notice, because plists fail silently (that's why the change, better sooner than later).  See updated examples for =org-node-filter-fn= etc.
...
If you're new to the whole concept, you don't really need to configure anything.

As with org-roam, the main things you use in day-to-day operation are two commands: "find" and "insert".  Pick some good keys and try them out, and you can come back to this README later---or never.
...
(I like F2, but as everyone aside from Xah Lee seems to hate F1-F12, maybe you'd prefer... let's say =M-s=?)
...
To browse other config options, type =M-x customize-group RET org-node RET=.
...
With that done, try the command substitutes:
- =org-node-find= instead of =org-roam-node-find=,
- =org-node-insert-link= instead of =org-roam-node-insert=
- =org-node-extract-subtree= insted of =org-roam-extract-subtree=

And there are other commands listed under [[https://github.com/meedstrom/org-node?tab=readme-ov-file#commands][Commands]].  Enjoy!

I'm guessing you also want [[https://github.com/meedstrom/org-node?tab=readme-ov-file#backlink-solutin-1-org-roam-buffer][org-roam-buffer backlinks]].  See that section.
