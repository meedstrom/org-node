...
Enable link completion in all =org-mode= buffers with:

#+begin_src elisp
(setq
 org-node-completion-everywhere t)
#+end_src

You'll need to disable =org-roam='s completion as well, if you're running =org-roam=:
