...
You may notice backlinks staying after you erased the forward link.  For performance reasons, org-node waits for idle before cleaning up.

Also due to this mechanism, org-roam-buffer's contextual display may shift to show irrelevant text as a file is edited; this too is corrected on idle.

*** Limitation: links with spaces in =ROAM_REFS=

Currently to seek reflinks, org-node just uses =org-link-plain-re=, which is not space-aware.  So =ROAM_REFS= like:

: :ROAM_REFS: [[https://romankogan.net/adhd/#False Dependency Chain]]

will not get reflinks.  I'll see about transiting to =org-link-any-re=, it just seems like a perf hit for a tiny use case.

# Oh... with any-re, you can make a datestamp a ref!  Maybe people do use that.
