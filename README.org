...
A *silent* obsoletion: links are not plists anymore, but =plist-get= is unhelpfully polite and returns nil when given invalid input.   Use e.g. =(org-mem-link-pos link)= instead of =(plist-get LINK :pos)=.
...
- Package org-node-fakeroam deprecated
  - New mode built-in: =org-node-roam-accelerator-mode=
  - New mode built-in: =org-mem-roamy-db-mode=
