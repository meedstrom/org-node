...
- *If you were not using org-roam*, maybe think of it as somewhat like [[https://github.com/alphapapa/org-recent-headings][org-recent-headings]] beefed-up to the extent that you won't need other methods of browsing, as long as you give IDs to all objects of interest.
...
(keymap-set global-map "M-s M-r" #'org-node-fakeroam-show-roam-buffer)
#+end_src
...
This could work by excluding a "project" tag or any note that has a TODO state:
...
Let's say you have a big archive file, fulla IDs, and you want all the nodes within out of sight.  Putting a =:ROAM_EXCLUDE: t= at the top won't do it.  Unlike in org-roam, *child ID nodes of an excluded node are not excluded!*  The =org-node-filter-fn= applies its ruleset to each node in isolation.
...
Or put something like =#+filetags: :wiki_exclude:= at the top of each file:
