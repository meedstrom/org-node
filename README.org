...
Yes.  I've got in my head an alternative solution to caching link data (primarily because =org-node-backlink-fix-all= is something of a PITA), and once it's in place, =CACHED_BACKLINKS= will be optional.  But I recommend opting in anyway, it grows on you!
...
| Get backlink IDs                |                                 | =(org-node-backlink-origins NODE)=                                                    |
...
| Get outline level               | =(org-roam-node-level NODE)=      | =(org-node-level NODE)=, also the boolean =:is-subtree=                                 |
...
| Get :PROPERTIES:                | =(org-roam-node-properties NODE)= | =(org-node-properties NODE)=                                                          |
