#+startup: content
[[https://melpa.org/#/org-node][file:https://melpa.org/packages/org-node-badge.svg]]
* org-node
** News [2024-Sep-02]
The git repo history was REWRITTEN!  You may have had to delete and reinstall the package.  Depends on your package manager.  Sorry about that.

*What was rewritten?*  I erased the =;; Package-Version:= comment line in all historical file contents, because of Emacs 30's new =(use-package :vc)= keyword, which defaults to installing the /oldest/ commit with a given Package-Version, not the /newest/ commit.

See a [[https://github.com/melpa/melpa/issues/2955#issuecomment-1519154736][loooong reelated discussion]].  Used to be if you wanted to stop bumping Package-Version, you could just stop, because Melpa would preferentially look at the git tag anyway.  But now, with Emacs 30?  I foresee many more full-repo rewrites in the coming years.

Lesson learned -- when you create a package, never commit any file containing a Package-Version line.  Doing so curses the repo for the rest of time.
...
NB: this workaround won't help the in-buffer completions provided by =org-node-complete-at-point-mode=, but with a light peppering of luck this isn't something you'll ever have to notice.  (Work on Issue 38 for principled fix)
...
- =org-node-grep=
  - (Requires [[https://github.com/minad/consult][consult]]) Grep across all known Org files.
    - Very useful combined with [[https://github.com/oantolin/embark][embark]]-export & [[wgrep][wgrep]], to search-and-replace a given string everywhere in every directory, for example to rename a tag once and for all.
