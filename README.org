* org-node
** News [2024-05-07]
- You'll now experience less interruptions in commands
- New variable to easily populate org-id: =org-node-extra-id-dirs=!
...
Do a =M-x org-roam-update-org-id-locations=.  With that done, try out the commands mentioned in [[https://github.com/meedstrom/org-node?tab=readme-ov-file#quick-start][Quick start]].  There's more under [[https://github.com/meedstrom/org-node?tab=readme-ov-file#toolbox][Toolbox]].  Enjoy!
...
Instead of the above, do some hax:
...
In principle, org-id.el is meant to be self-managing.  When Emacs sees an ID, it's added to the table.  In real-world practice, you can't always rely on that, so it provides settings like =org-id-extra-files=.

I've found these settings quite unsatisfactory -- you need a long init snippet to set them correctly (see [[https://github.com/meedstrom/org-node?tab=readme-ov-file#appendix-iii-taking-ownership-of-org-id][Appendix III]]).  So org-node has an additional way to feed data to org-id.  Configure =org-node-extra-id-dirs=.

As a bonus, it has an exclusion ruleset =org-node-extra-id-dirs-exclude= if you need it.

*Org-roam users* can use =M-x org-roam-update-org-id-locations=, but to avoid having to do it again on every init, also type =M-: (org-id-locations-save)= afterwards.
...
2. Take the variable =org-roam-mode-sections=.  Under any ordinary Emacs Lisp package, this would just be a list of functions.  But in fact, you can add to it a cons cell of a function plus the arguments to pass to it.  I like programmability, but this is ... oriented towards people who aren't programmers, I think.
   ...
   - It does make the org-roam source code a slower read (a [[http://weblog.raganwald.com/2004/10/beware-of-turing-tar-pit.html][Turing tarpit]]).  You scratch your head and ask "Why is it made that way?"  Then you see, and you say "Ah, but /I/ don't need that!"  Well, maybe someone does.
