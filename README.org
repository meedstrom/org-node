...
With that done, try the command =org-node-find= instead of =org-roam-node-find=, and other such equivalents listed in [[https://github.com/meedstrom/org-node#commands][Commands]]. Enjoy!

You may also want [[https://github.com/meedstrom/org-node#1-see-backlinks-in-the-org-roam-buffer-window][org-roam-buffer backlinks]].
...
You have two different ways to see backlinks:

**** 1. See backlinks in the org-roam-buffer window

Want the good old =M-x org-roam-buffer-toggle=?

You can tell org-node to share its data with org-roam.  For a first-time run, type =M-x org-node-feed-roam-db= (slow!).  Then add this init snippet to keep the database updated over time:
...
It is also possible to cut out the DB altogether.  Instead of the above, add this advice:
...
**** 2. See backlinks on a subtree property line

I love this solution, because I rarely have space for a second window to show org-roam's backlinks.

You can let org-node add a =BACKLINKS= property to all nodes.  For a first-time run, type =M-x org-node-backlink-fix-all=.

Turn on =M-x org-node-backlink-mode=, a global minor mode that keeps these properties updated.  Init snippet:
