...
If you need TRAMP, use org-roam, which is made to /not/ re-access files or directories so often anyway (which can be slow anyway over a remote connection), in favor of trusting its own DB.
