...
Org-node depends on =org-mem-updater-mode= from [[https://github.com/meedstrom/org-mem][org-mem]].
...
See [[#appendix-ii-how-to-rollback][How to rollback]] at the end of this readme.
...
- =org-node-find= (~M-o n f~)
  - Tip: Consider binding a short key like ~M-o f~
- =org-node-insert-link= (~M-o n i~)
  - Tip: Consider binding a short key like ~M-o i~
...
Since [[https://github.com/meedstrom/org-mem][org-mem]] only looks at saved files on disk, new notes only "appear" after save.  So it can make for a smoother experience to enable =auto-save-visited-mode=.
...
FWIW, org-node used to try to be smarter and also cache notes in unsaved buffers, but that quickly became difficult to reason about, especially wrt. deciding whether to update a backlink or not.
...
(For background, see [[#appendix-iv-what-are-roam_refs][What are ROAM_REFS?]] at the end of this README.)
...
3. Try the command =org-node-seq-dispatch= (~M-o n s~)
  - Tip: Consider binding a shorter key ~M-o s~
