...
** Limitation: org-ref
We support the builtin @citations, not (yet) org-ref &citations, since amending =org-link-plain-re= to match them slows down my scan about 50%.  But, you can still find nodes with e.g. =:ROAM_REFS: &citekey=, just not see backlinks based on &citekey.
