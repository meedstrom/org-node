...
(You could read [[https://edstrom.dev/cdlxm/how-to-downgrade-rollback-an-emacs-package][How to Downgrade/Rollback an Emacs Package]], but org-node makes it simpler because it creates branches like v1.6 and v1.7, for each minor version series.)
...
The extension [[https://github.com/meedstrom/org-node-fakeroam][org-node-fakeroam]] has been tightly coupled with contemporary versions of org-node, but hopefully I've made it warn at load time if it sees a version mismatch.  Then you can use a similar procedure as above.
