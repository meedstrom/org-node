...
- =org-node-find=
  - Equivalent to =org-roam-node-find=
- =org-node-insert-link=
  - Equivalent to =org-roam-node-insert=
...
*** Getting the org-roam-buffer back

To have =M-x org-roam-buffer-toggle= do the right thing, first eval an advice:
...
We can do something similar, though our pseudo-outline path does not include the subtrees without IDs... only those with, and it makes cache rebuild 50% slower.  It works well, but I'm marking it experimental because I'm not sure how much further to go before I'm just reinventing org-ql.
...
As you might infer, all the stuff in an org-node plist was collected by Ripgrep, and for the rest, a "visit-get" function like =org-node--visit-get-properties= has to visit the target.  It's a bad idea to call a "visit-get" from your custom =org-node-filter-fn=, as that horribly slows down the cache rebuilds -- to the speed of an =(org-roam-db-sync 'force)=, in fact.

# For an overview of what org-roam-db persists to disk, see its EmacSQL table schemata at [[https://github.com/org-roam/org-roam/blob/v2.2.2/org-roam-db.el#L229][org-roam-db--table-schemata]].
