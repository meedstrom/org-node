...
#+begin_src elisp
(indexed-roam-mode) ;; For ROAM_REFS
...
(use-package org-node
  :config
  (setq indexed-check-org-id-locations t)
  (indexed-updater-mode)
  (indexed-roam-mode) ;; Support ROAM_REFS
  (org-node-cache-mode))
#+end_src

If you're *coming here from org-roam* and still want to use some of its features or just feel at home, try
...
(use-package org-node
  ...
  ;; Tip (not related to org-node): uncomment to further speed up saving
