...
Changes
- Completions are now "rich" by default -- to revert this, set =(setq org-node-affixate-fn #'org-node-affix-bare)=
...
Some example capture templates follow. The secret sauce is =(function org-node-capture-target)=.
...
#+begin_src elisp
(setq org-node-extra-id-dirs '("/home/kept/notes/"
                               "/home/kept/project1/"
                               "/home/kept/project2/")
#+end_src
...
That's just the nature of completion.  Other packages such as Org-roam have the same limitation.  Much can be said for embracing the uniqueness constraint, and Org-node will print messages telling you about title collisions (controlled by the option =org-node-warn-title-collisions=).
...
NB: this workaround won't help the in-buffer completions provided by =org-node-capf-mode=, but hopefully you won't need it often.
...
(Actually, that design caused me no end of problems.  I got an out-of-sync DB all the time, it's the challenge of cache invalidation. I think it's saner to check the disk files more often!)
