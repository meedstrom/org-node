...
Finally, make sure the underlying org-id knows about the files org-roam knows.  You'd think it would, but that isn't a given!  Do either one of these:

1. Run =M-x org-roam-update-org-id-locations=, then =M-: (org-id-locations-save)=
...
and configure =org-node-series-defs= if needed.
