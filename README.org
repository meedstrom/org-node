...
A *silent* obsoletion: links are not plists anymore, but =plist-get= is unhelpfully polite and returns nil when given invalid input.   Use e.g. =(org-mem-link-pos LINK)= instead of =(plist-get LINK :pos)=.  Also, a link is a citation if =(org-mem-link-citation-p LINK)=.
