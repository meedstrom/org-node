...
You can use [[https://github.com/bastibe/org-journal][org-journal]] for the concept of "dailies".  Example settings that emulate the default style of org-roam dailies:
...
- org-journal-previous-entry / org-journal-next-entry
- org-journal-open-current-journal-file

Org-node does not yet have a capture target for dailies.

** Tip: On very slow filesystems

I hear that on Termux on Android, filesystem access can be so slow that it's a pain to use org-roam-dailies as well as org-journal. ([[https://github.com/meedstrom/org-node/issues/24#issuecomment-2278605819][19 seconds just to list files under a directory]]!)

Good news!  As an org-node user, you can speed up a lot of their functions by making them just look up org-node tables.  One or more of these overrides may interest you:

#+begin_src elisp
(advice-add #'org-journal--list-files :override
            #'org-node-list-journal-files))

(advice-add #'org-roam-dailies--list-files :override
            #'org-node-list-roam-daily-files)

;; (Good even if fs is not an issue; 850ms to 2ms on my machine)
(advice-add #'org-roam-list-files :override
            #'org-node-list-roam-files)
#+end_src

** Tip: Word-wrap in the Roam buffer
This has nothing to do with org-node, but I actually didn't know this for ages.  If you don't hard-wrap ([[https://github.com/org-roam/org-roam/issues/1862][org-roam#1862]]) but prefer visual-line-mode or similar, you have to enable them yourself:
