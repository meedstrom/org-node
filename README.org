...
If you need TRAMP, use org-roam, which is made to /not/ re-access files or directories so often anyway (which would also be slow over a network connection), in favor of trusting its own DB.
