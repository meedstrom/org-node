...
* News [2025-03-03 Mon] :noexport:

Version 2.0 is out, with two major features:
- backlink drawers (providing an alternative to org-super-links)
- context buffers (providing an alternative to org-roam's buffers)

*If you were using version 1.8.7 or older (November 2024), you'll have a smoother ride if you first upgrade to the latest in [[https://github.com/meedstrom/org-node/tree/v1.9][v1.9 branch]],* deal with any transitional warnings, /then/ upgrade to the latest version on [[https://github.com/meedstrom/org-node/branches][main]].

Or else just check out that you're not using any of [[https://github.com/meedstrom/org-node/blob/v1.9/org-node-changes.el][these aliases]].

** Backlinks drawer

You can choose whether =org-node-backlink-mode= should use multiline drawers or a single property line.  The drawers are the same idea as [[https://github.com/toshism/org-super-links][org-super-links]].  I.e. instead of this:
...
The two old commands =org-node-backlink-fix-all-files=  and =org-node-backlink-regret= are replaced by four, hopefully self-explanatory:

- Command =org-node-backlink-mass-update-drawers=
- Command =org-node-backlink-mass-update-props=
- Command =org-node-backlink-mass-delete-drawers=
- Command =org-node-backlink-mass-delete-props=

** Context buffers

The new context buffer replaces much of [[https://github.com/meedstrom/org-node-fakeroam][org-node-fakeroam]]'s hacks, and with them, org-roam itself.
...
- Minor mode =org-node-context-follow-global-mode=
...
- Variable =org-node-context-refresh-hook=
  - replaces =org-roam-preview-postrender-functions=

You no longer need these:
- Minor mode =org-node-fakeroam-jit-backlinks-mode=
- Minor mode =org-node-fakeroam-fast-render-mode=

Some technical differences from org-roam's buffer:
- Variable =org-node-context-postprocess-hook= -- like =org-roam-buffer-postprocess-functions=, but runs in a buffer instead of passing a string
- No grep section (yet?)
...
And because someone will ask, the remaining utility in org-node-fakeroam boils down to:

- Using org-roam-capture
  : (setq org-node-creation-fn 'org-node-fakeroam-new-via-roam-capture)

- Keeping org-roam's DB updated, so that extensions that rely on it can continue to work
  : (org-node-fakeroam-db-feed-mode)

  - Note that the performance leaves something to be desired.

** Incompatible changes

1. Removed a number of [[https://github.com/meedstrom/org-node/blob/v1.9/org-node-changes.el][deprecated aliases]].
...
5. Removed user option =org-node-backlink-aggressive=, in favor of =org-node-backlink-lazy=, with inverted meaning.
...
Bonus: you can still use a command =M-x org-node-backlink-fix-all-files= to add missing backlinks in bulk.  That may be useful to start you off on your life with org-super-links.

However, when you have pre-existing drawers -- *MAKE A FULL BACKUP!*

Org-node has a different usage in mind than org-super-links, and you may be accustomed to having old manually formatted and sorted drawers.  Running =org-node-backlink-fix-all-files= *may re-sort your backlinks and re-format their appearance into something you don't want*; double-check the following options!

- =org-node-backlink-drawer-sorter=
- =org-node-backlink-drawer-formatter=

Additionally, any lines that don't contain a valid link are simply *deleted*.
