...
- org-node is now async and multi-threaded, speeding it up even more!
  ...
  If anything breaks, try nulling the user option =org-node-perf-multicore=, or revert to the "before-refactor" branch, which can be done in the Straight package expression like:
...
| Asynchronous                       | no          | yes                | yes (very async ;)   |
...
With that done, try out the commands mentioned in [[https://github.com/meedstrom/org-node?tab=readme-ov-file#quick-start][Quick start]].  There's more under [[https://github.com/meedstrom/org-node?tab=readme-ov-file#commands][Commands]].  Enjoy!
