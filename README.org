...
Bonus 1: if you often have reason to full-reset the DB, there is a faster command than =C-u M-x org-roam-db-sync=!  Try =M-x org-node-fakeroam-db-rebuild=. Takes me 6 seconds instead of 179 seconds.

Bonus 2: if the Roam backlinks buffer is sometimes slow, the minor mode =M-x org-node-fakeroam-fast-render-mode= should speed it up.
...
(org-node-fakeroam-redisplay-mode) ;; always show backlinks relevant to point
#+end_src
