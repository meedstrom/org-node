...
(use-package org-node
  :init
  ;; Optional; note that doing this on :init loads the package.
...
NOTE: To be clear, this mode never generates new IDs.  That's your own business.  This only adds/edits =:BACKLINKS:= properties.

It's the same you observe in the backlink buffers from earlier: *not all links create a backlink,* only links located in an entry that has or inherits an ID.  After all, there must be something to link back to.
...
(setq org-node-filter-fn
      (lambda (node)
        (string-search "/my-personal-wiki/" (org-mem-entry-file node))))
#+end_src
...
Putting a =:ROAM_EXCLUDE: t= at the top won't do it, because unlike in org-roam, *child ID nodes of an excluded node are not excluded!*  The default value of org-node-filter-fn is =org-node-filter-no-roam-exclude-p=,
but it applies to each node in isolation.
