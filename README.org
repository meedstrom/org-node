* org-node
** News [2024-05-07]
- New variable for easily populating org-id: =org-node-extra-id-dirs=
...
Also, either run =M-x org-roam-id-update-id-locations=, or add your =org-roam-directory= to the following variable.  It shouldn't be necessary, but upstream org-id has no real convenient setting for this...
...
I find unsatisfactory the config options in org-id (see [[https://github.com/meedstrom/org-node?tab=readme-ov-file#appendix-iii-taking-ownership-of-org-id][Appendix III]]), so org-node has an additional way to feed data to org-id, so we won't run into "ID not found" situations.
