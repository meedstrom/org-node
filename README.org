...
My life can be divided into two periods "Before Org-Roam" and "After Org-Roam".  I crossed a kind of gap once I got in the habit of linking between my notes.  It's odd to remember when I didn't have that habit, and just relied on browsing hierarchies of subtrees and the occasional file-link -- what a strange way to do things!
...
Here's what org-roam invented.  It turns out that if you limit the search hits to just those files and subtrees you've deigned to assign an org-id -- which roughly maps to everything you've ever thought it was worth linking to or from (or will be worth linking) -- it filters out the noise excellently.
...
I found org-roam too slow, so I made [[https://github.com/meedstrom/quickroam][quickroam]].  And that idea spun off into this package, a standalone, bizarro alternative.

- *If you were using org-roam,* you don't need to migrate.  It's exactly the same on-disk format: "notes" are identified by their org-id.  Think of it as an extra set of commands for working with your notes.

  It even has shims to let you use org-roam's own features, like the rich backlinks buffer, without syncing its SQLite DB!
...
- If you were *not* using org-roam, think of it as a limited, focused alternative to org-ql or something.  If you were the sort of person to prefer org-id links over file links or any other type, you're in the right place: this basically lets you forget about filenames and hierarchies and directory structures -- as long as you've given an org-id to something, you can find it later.
...
Yes.  Yes.  But I've come to like the backlinks property anyway, so it's not my itch to scratch.
